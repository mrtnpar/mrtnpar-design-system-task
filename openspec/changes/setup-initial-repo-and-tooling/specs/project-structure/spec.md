...
The repository MUST follow a clear directory structure that supports atomic design principles and platform separation.
...
And `src/components/` contains all UI components
...
Given components follow atomic design
Then components are organized by type:
- `src/components/atoms/` - basic UI elements
- `src/components/molecules/` - composed atoms
- `src/components/organisms/` - complex composed components
